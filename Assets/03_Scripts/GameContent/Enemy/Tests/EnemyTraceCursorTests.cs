...
namespace Pawntom.Enemy.Tests
{
    ...
    /// <para>
    /// 여기서는 <see cref="EnemyBrain"/> 을 한 번도 만들지 않는다. 커서는 좌표와 id 만 다루므로
    /// 두뇌도 씬도 없이 결정적으로 검증된다.
    /// </para>
    /// </summary>
    public sealed class EnemyTraceCursorTests
    {
        ...
        {
            EnemyTraceCursor cursor = new EnemyTraceCursor();
            List<EnemyTraceCandidate> candidates = new List<EnemyTraceCandidate>
            {
                Candidate(1, 20f, 100),
                Candidate(2, 15f, 200)
            };

            EnemyTraceStep step = cursor.Advance(Vector3.zero, candidates, Radius, Reach);
        ...
        public void Criterion3_ReachingTarget_ReportsConsumedId()
        {
            ...
            // 첫 호출 — 목표만 잡는다.
            EnemyTraceStep first = cursor.Advance(Vector3.zero, candidates, Radius, Reach);
            Assert.IsTrue(first.HasTarget);
            Assert.IsFalse(first.HasConsumed, "아직 멀리 있는데 소비했다");

            // 개가 목표 코앞까지 갔다.
            EnemyTraceStep second = cursor.Advance(new Vector3(4.5f, 0f, 0f), candidates, Radius, Reach);
        ...
        public void Criterion4_ConsumedCandidate_IsNotRetargetedInSameCall()
        {
            ...
            EnemyTraceStep second = cursor.Advance(Vector3.zero, candidates, Radius, Reach);
        ...
        public void Criterion5_TargetLeavingRadius_ReleasesTarget()
        {
            ...
            EnemyTraceStep step = cursor.Advance(new Vector3(-5f, 0f, 0f), candidates, Radius, Reach);
        ...
        {
            EnemyTraceCursor cursor = new EnemyTraceCursor();
            List<EnemyTraceCandidate> empty = new List<EnemyTraceCandidate>();

            EnemyTraceStep step = default(EnemyTraceStep);
        ...
        public void ZeroDetectionRadius_PicksNothing()
        {
            ...
            EnemyTraceStep step = cursor.Advance(Vector3.zero, candidates, 0f, Reach);
        ...
        public void ApproachingOverManyTicks_ConsumesOnlyOnArrivalTick()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                EnemyTraceStep step = cursor.Advance(
            ...
            EnemyTraceStep arrival = cursor.Advance(new Vector3(4f, 0f, 0f), candidates, Radius, Reach);
        ...
        public void TraceCursor_OriginRaisedByBaseOffset_ConsumesTraceDirectlyBelow()
        {
            ...
            EnemyTraceStep first = cursor.Advance(DogOrigin, candidates, Radius, Reach);
            Assert.IsTrue(first.HasTarget, "사전 조건: 바로 아래 흔적을 목표로 잡는다");

            EnemyTraceStep second = cursor.Advance(DogOrigin, candidates, Radius, Reach);
        ...
        public void TraceCursor_HeightGap_UsesHorizontalDistanceForRadius()
        {
            ...
            EnemyTraceStep inside = new EnemyTraceCursor().Advance(DogOrigin, candidates, 8f, Reach);
            Assert.IsTrue(inside.HasTarget, "수평 5m 는 반경 8 안이다");
            Assert.IsFalse(inside.HasConsumed, "수평 5m 인데 도달로 봤다");

            EnemyTraceStep outside = new EnemyTraceCursor().Advance(DogOrigin, candidates, 4f, Reach);
            Assert.IsFalse(outside.HasTarget, "수평 5m 는 반경 4 밖이다");

            // 경계 5.2 — 3D 로 재면 sqrt(5^2 + 2.7^2) = 5.68 이라 여기서 갈린다.
            EnemyTraceStep boundary = new EnemyTraceCursor().Advance(DogOrigin, candidates, 5.2f, Reach);
        ...
        public void TraceCursor_HeightGap_StillPicksNewestCandidate()
        {
            ...
            EnemyTraceStep step = cursor.Advance(DogOrigin, candidates, Radius, Reach);
        ...
        public void TraceCursor_SameTickWithHeightGap_PicksHorizontallyNearer()
        {
            ...
            List<EnemyTraceCandidate> candidates = new List<EnemyTraceCandidate>
            {
                ...
                new EnemyTraceCandidate(42, new Vector3(3.5f, DogY, 0f), 150)
            };
        ...
        private static EnemyTraceCandidate Candidate(int id, float distance, int createdTick)
        {
            return new EnemyTraceCandidate(id, new Vector3(distance, 0f, 0f), createdTick);
        }

        /// <summary>바닥에 붙은 흔적 하나. y 는 실제 스폰 높이 <see cref="TraceY"/> 를 쓴다.</summary>
        private static EnemyTraceCandidate GroundTrace(int id, float x, float z, int createdTick)
        {
            return new EnemyTraceCandidate(id, new Vector3(x, TraceY, z), createdTick);
        }
    }
}
