...
{
    /// <summary>
    /// TASK-014 5.4 — 대상 집합(<see cref="EnemyTargetSet"/>)과
    /// 등록자(<see cref="EnemyTargetRegistrar"/>)의 대기 드레인 검증.
    /// <para>
    /// (나) 묶음은 1차 스펙 반려 사유였던 실패 경로다 —
    /// 대기 목록 드레인이 빠지면 "등록자 먼저 → 창구 나중" 테스트가 반드시 실패한다.
    /// </para>
    /// </summary>
    public sealed class EnemyTargetSetTests
    {
        /// <summary>
        /// <see cref="IEnemyTargetRegistry"/> 의 가짜 구현. 받은 것을 담기만 한다.
        /// 등록자가 구체 제공자를 모른다는 사실 자체가 이 가짜를 끼울 수 있는 근거다.
        /// </summary>
        private sealed class FakeRegistry : IEnemyTargetRegistry
        {
        ...
        public void SetUp()
        {
            ...
            Assert.IsNull(EnemyTargetRegistrar.Registry, "사전 조건: 창구가 비어 있다");
        }
        ...
        {
            EnemyTargetSet set = new EnemyTargetSet();
        ...
        public void TargetRegistrar_EnabledBeforeAttach_IsDrainedIntoRegistry()
        {
            ...
            Assert.IsNull(EnemyTargetRegistrar.Registry, "사전 조건: 아직 창구가 없다");

            // ② 창구를 나중에 꽂는다.
            FakeRegistry fake = new FakeRegistry();
            EnemyTargetRegistrar.AttachRegistry(fake);
        ...
        public void TargetRegistrar_Disabled_UnregistersFromRegistry()
        {
            ...
            EnemyTargetRegistrar registrar = EnableRegistrar(host);
        ...
        public void TargetRegistrar_DisabledWithoutRegistry_DoesNotThrow()
        {
            ...
            EnemyTargetRegistrar registrar = EnableRegistrar(host);
            Assert.IsNull(EnemyTargetRegistrar.Registry, "사전 조건: 창구가 없다");
        ...
        public void TargetRegistrar_DetachRegistry_WithForeignRegistry_KeepsCurrent()
        {
            ...
            EnemyTargetRegistrar.AttachRegistry(owner);
            EnemyTargetRegistrar.DetachRegistry(stranger);

            Assert.AreSame(owner, EnemyTargetRegistrar.Registry, "남의 창구는 뺄 수 없다");

            EnemyTargetRegistrar.DetachRegistry(owner);
            Assert.IsNull(EnemyTargetRegistrar.Registry, "당사자는 뺄 수 있다");
        }
        ...
        public void TargetRegistrar_AttachNull_KeepsCurrentRegistry()
        {
            ...
            EnemyTargetRegistrar.AttachRegistry(null);

            Assert.AreSame(owner, EnemyTargetRegistrar.Registry, "null 은 아무것도 하지 않는다");
        }
        ...
        {
            PropertyInfo property = typeof(EnemyTargetRegistrar).GetProperty(
        ...
        {
            Assert.IsTrue(
                typeof(IEnemyTargetRegistry).IsAssignableFrom(typeof(SceneTargetProvider)),
                "제공자가 IEnemyTargetRegistry 를 구현하지 않는다");
        }
        ...
        /// </para>
        /// </summary>
        private static EnemyTargetRegistrar EnableRegistrar(GameObject host)
        {
            EnemyTargetRegistrar registrar = host.AddComponent<EnemyTargetRegistrar>();
            InvokeLifecycle(registrar, "OnEnable");
            return registrar;
        }

        private static void DisableRegistrar(EnemyTargetRegistrar registrar)
        {
            InvokeLifecycle(registrar, "OnDisable");
        }

        private static void InvokeLifecycle(EnemyTargetRegistrar registrar, string methodName)
        {
            MethodInfo method = typeof(EnemyTargetRegistrar).GetMethod(
        ...
        {
            EnemyTargetRegistrar.DetachRegistry(EnemyTargetRegistrar.Registry);

            FakeRegistry drain = new FakeRegistry();
            EnemyTargetRegistrar.AttachRegistry(drain);
            EnemyTargetRegistrar.DetachRegistry(drain);
        }
