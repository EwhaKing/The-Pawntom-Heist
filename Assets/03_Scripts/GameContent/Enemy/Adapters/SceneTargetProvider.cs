...
namespace Pawntom.Enemy.Adapters
{
    ...
    /// </para>
    /// <para>
    /// 런타임에 스폰되는 대상(플레이어 등)은 <see cref="EnemyTargetRegistrar"/> 컴포넌트를
    ...
    public sealed class SceneTargetProvider : MonoBehaviour, IEnemyTargetProvider, IEnemyTargetRegistry
    {
        ...
        private readonly EnemyTargetSet _set = new EnemyTargetSet(8);
        ...
        private void Awake()
        {
            ...
            EnemyTargetRegistrar.AttachRegistry(this);
        }

        private void OnDestroy()
        {
            // 자기가 꽂은 경우에만 뺀다 — 정적 Instance 처리와 같은 방식이다.
            EnemyTargetRegistrar.DetachRegistry(this);
