...
namespace Pawntom.Enemy.Core
{
    ...
    {
        // 속도의 정본은 이 두 필드다. 씬의 Nav Mesh Agent 컴포넌트에 보이는 Speed 는 파생값이라,
        // 이동 명령이 나갈 때마다 아래 값으로 덮어써진다. 아래 툴팁에 같은 내용을 적어 둔다.
        [Header("이동 속도")]
        ...
        [Tooltip("이동 중 몸이 도는 속도(도/초). 경계 중 조준 회전(Alert Turn Speed Degrees)과는 별개다")]
        [SerializeField] private float _moveTurnSpeedDegrees = 540f;
        ...
        [SerializeField] private float _waypointWaitSeconds = 1.0f;
        ...
        [SerializeField] private float _investigateGiveUpSeconds = 20f;

        [Tooltip("추격 중 시야를 잃고 조사로 복귀하기까지의 시간(초). 기획서 명시값 5")]
        [SerializeField] private float _chaseLoseSightSeconds = 5f;
        ...
        [Header("경계")]
        [Tooltip("경계 중 목표 쪽으로 몸을 돌리는 속도(도/초). 0 이하면 돌지 않는다")]
        [SerializeField] private float _alertTurnSpeedDegrees = 360f;

        [Tooltip("경계 진입 후 제자리를 지키는 시간(초). 이 시간이 지나면 시야가 있는 즉시 추격한다. 0이면 하울링을 시작하자마자 달려든다")]
        [SerializeField] private float _alertHoldSeconds = 0f;
        ...
        [SerializeField] private float _investigateWanderRadius = 3f;

        [Tooltip("배회 지점에 도달한 뒤 다음 지점을 고르기까지의 대기 시간(초)")]
        [SerializeField] private float _investigateWanderIntervalSeconds = 2f;

        [Tooltip("고른 지점이 갈 수 없는 곳일 때 근처에서 갈 수 있는 곳을 찾는 허용 거리(m)")]
        [SerializeField] private float _investigateWanderSampleDistance = 1.5f;
        ...
        public float MoveTurnSpeedDegrees { get { return _moveTurnSpeedDegrees; } }

        public float WaypointArriveDistance { get { return _waypointArriveDistance; } }

        public float WaypointWaitSeconds { get { return _waypointWaitSeconds; } }
        ...
        public float InvestigateGiveUpSeconds { get { return _investigateGiveUpSeconds; } }

        public float ChaseLoseSightSeconds { get { return _chaseLoseSightSeconds; } }

        public float HowlDurationSeconds { get { return _howlDurationSeconds; } }

        public float HowlRadius { get { return _howlRadius; } }

        public float AlertTurnSpeedDegrees { get { return _alertTurnSpeedDegrees; } }

        public float AlertHoldSeconds { get { return _alertHoldSeconds; } }
        ...
        public float InvestigateWanderRadius { get { return _investigateWanderRadius; } }

        public float InvestigateWanderIntervalSeconds { get { return _investigateWanderIntervalSeconds; } }

        public float InvestigateWanderSampleDistance { get { return _investigateWanderSampleDistance; } }
    }
}
